...
class BicubicDownSample(nn.Module):
    ...
    def __init__(self, factor=4, cuda=False, padding='reflect'):
        super().__init__()
        self.factor = factor
        size = factor * 4
    ...
    def forward(self, x, nhwc=False, clip_round=False, byte_output=False):
        ...
        filters1 = self.k1.type('torch{}.FloatTensor'.format(''))
        filters2 = self.k2.type('torch{}.FloatTensor'.format(''))
        ...
        if byte_output:
            return x.type('torch.ByteTensor'.format(''))
        else:
            return x
