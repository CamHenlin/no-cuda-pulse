...
class PULSE(torch.nn.Module):
    def __init__(self, cache_dir, verbose=True):
        ...
        self.synthesis.load_state_dict(torch.load('synthesis.pt', map_location=torch.device('cpu')))
        ...
        if Path("gaussian_fit.pt").exists():
            print("\tLoading gaussian_fit Network")
            self.gaussian_fit = torch.load("gaussian_fit.pt", map_location=torch.device('cpu'))
        else:
            ...
            mapping.load_state_dict(torch.load('mapping.pt', map_location=torch.device('cpu')))
    ...
    def forward(self, ref_im, seed, loss_str, eps, noise_type, num_trainable_noise_layers, tile_latent, bad_noise_layers, opt_name, learning_rate, steps, lr_schedule, save_intermediate, **kwargs):
        ...
        if (tile_latent):
            latent = torch.randn(
                (batch_size, 1, 512), dtype=torch.float, requires_grad=True, device='cpu')
        else:
            latent = torch.randn(
                (batch_size, 18, 512), dtype=torch.float, requires_grad=True, device='cpu')
        ...
        for i in range(18):
            ...
            if (noise_type == 'zero' or i in [int(layer) for layer in bad_noise_layers.split('.')]):
                new_noise = torch.zeros(res, dtype=torch.float, device='cpu')
